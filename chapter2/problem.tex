...
\subsection {Multitasking with BCI machine}\cite{tow}
...
\subsection {Real-time control of a robotic arm using a brain-computer interface}\cite{asy}
...
\subsection {Visual stimuli for the P300 brain-computer interface: A comparison of white/grey and green/blue flicker matrices}\cite{vis}
...
\newpage
\subsection {Validation of the Emotiv EPOC® EEG gaming system for measuring research quality auditory ERPs\cite{val}}
...
\subsection {c-VEP Brain-Computer Interface (BCI)\cite{onl}}
...
\begin{figure}[h]
	\centering
  	...
  	\caption{A: The grey area shows the 32 target stimuli with the number referring to the number of the target. The stimuli in the white area are the complementary, which are synchronized to the target with the same number. B: Modulation sequence for the first 5 targets. The sequence of a target is shifted by 2 bit in respect to its preceding target.}
\end{figure}

\subsection {Human EEG responses to 1-100 Hz flicker\cite{hum}}
...
\begin{figure}[h]
	\centering
  	\includegraphics[scale = 0.25]{chapter2/26.pdf}
...
\begin{figure}[h]
	\centering
  	\includegraphics[scale = 0.25]{chapter2/27.pdf}
...
\subsection {SSVEP-based brain computer interface using the Emotiv EPOC\cite{ssv}}
...
\subsection {A Survey of Stimulation Methods Used in SSVEP-Based BCIs\cite{sur}}
