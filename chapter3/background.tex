...
Standard deviation(SD) is measuring a distribution of the collection of data.if the SD is high, the distribution is high. if the SD is low, the distribution is low.
...
The High-pass filter only allow the frequency signal that is higher than cut-off frequency
\begin{figure}[ht]
	\centering
...
The Band-pass filter only allow the frequency signal that is in the specified range
\begin{figure}[ht]
	\centering
...
\newpage
\subsection{EEG band}
EEG band determined as a fixed range of wave frequency and amplitudes over a time scale. In Scientific field classify into 5 types. that are,
